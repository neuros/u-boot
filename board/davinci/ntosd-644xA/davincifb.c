...
static int davincifb_set_par(struct fb_info *info)
{
	...
	// otherwise, defualting to NTSC
	if ((v->xres == DISP_XRESD1 ) && (v->yres == DISP_YRESD1))
	{
		dmparams.output = NTSC;
		v->vmode = FB_VMODE_INTERLACED;
		printf("FBDEV: Configuring for NTSC\n");
	}
	else if ((v->xres == DISP_XRESD1 ) && (v->yres == 576))
	{
		dmparams.output = PAL;
		v->vmode = FB_VMODE_INTERLACED;
		printf("FBDEV: Configuring for PAL\n");
	}
	else if ((v->xres == DISP_XRES720P ) && (v->yres == DISP_YRES720P))
	{
		dmparams.output = LCD_720P;
		v->vmode = FB_VMODE_NONINTERLACED;
		printf("FBDEV: Configuring for 720P\n");
	}
		 ...
		 ((v->yres == DISP_YRES1080I) || (v->yres == 1080)))
	{
		...
		v->vmode = FB_VMODE_INTERLACED;
		printf("FBDEV: Configuring for 1080I\n");
	}
		...
		|| ((v->xres == 640) && (v->yres == 480))) // allow runtime switching
	{
		...
		dmparams.format = RGB;
		v->vmode = FB_VMODE_NONINTERLACED;
		printf("FBDEV: Configuring for LCD\n");
	}
	else
	{
		printf("Unrecognized output format (defaulting to NTSC)\n");
		dmparams.output = NTSC;
		v->vmode = FB_VMODE_INTERLACED;
		v->xres = 720;
		v->yres = 480;
			   ...
			   unsigned long arg)
{
	...
	case FBIO_SETLCDTIMEPARAMS:
		printf("FBDEV: Configuring for VGA\n");
	...
	case FBIO_SET_VIDEODAC:
		if (arg >= 0 && arg <= DAC_MAX) {
			dmparams.DAC = arg;
			return 0;
		} else
			return -EINVAL;
...
**
*/
static void * fbmalloc(int size, int flags)
{
	static void *memaddr = 0x81200000;	/* we should have some memory here */
	void *memp;
	int allocSize;

	/* Always allocate multiples of 256 bytes */
	allocSize = ((size + 0xff) & 0xffffff00) + 0x100;
	memp = memaddr;		/* we will return this pointer */
	memaddr += allocSize;
	return memp;
}
		     ...
		     unsigned long fb_size, char *fbname)
{
	...
	w = fbmalloc(sizeof(struct dm_win_info), GFP_KERNEL);
	if (!w) {
...
static void davincifb_lcd_rgb_config(int on)
{
	...
	if (on) {
		// Set new baseX and baseY
		dispc_reg_out(OSD_BASEPX, BASEXVGA);
		dispc_reg_out(OSD_BASEPY, BASEYVGA);

		/* Reset video encoder module */
		dispc_reg_out(VENC_VMOD, 0);

		// set LCD_OE to be active, enable RGB666 mode
		dispc_reg_merge(PINMUX0, 0x01400000, 0x01400000);  //rgb666
		...
		// turn off interlacing on all windows
		dispc_reg_merge(OSD_VIDWINMD, 0, OSD_VIDWINMD_VFF0);
		dispc_reg_merge(OSD_VIDWINMD, 0, OSD_VIDWINMD_VFF1);
		dispc_reg_merge(OSD_OSDWIN0MD, 0, OSD_OSDWIN0MD_OFF0);
		dispc_reg_merge(OSD_OSDWIN1MD, 0, OSD_OSDWIN1MD_OFF1);


		// Set VID0 window  origin and size
		dispc_reg_out(OSD_VIDWIN0XP, 0);
		dispc_reg_out(OSD_VIDWIN0YP, 0);
		dispc_reg_out(OSD_VIDWIN0XL, DISP_XRESVGA);
		dispc_reg_out(OSD_VIDWIN0YL, DISP_YRESVGA);

		// Set VID1 window  origin and size
		dispc_reg_out(OSD_VIDWIN1XP, 0);
		dispc_reg_out(OSD_VIDWIN1YP, 0);
		dispc_reg_out(OSD_VIDWIN1XL, DISP_XRESVGA);
		dispc_reg_out(OSD_VIDWIN1YL, DISP_YRESVGA);

		// Set OSD0 window  origin and size
		dispc_reg_out(OSD_OSDWIN0XP, 0);
		dispc_reg_out(OSD_OSDWIN0YP, 0);
		dispc_reg_out(OSD_OSDWIN0XL, DISP_XRESVGA);
		dispc_reg_out(OSD_OSDWIN0YL, DISP_YRESVGA);

		// Set OSD1 window  origin and size
		dispc_reg_out(OSD_OSDWIN1XP, 0);
		dispc_reg_out(OSD_OSDWIN1YP, 0);
		dispc_reg_out(OSD_OSDWIN1XL, DISP_XRESVGA);
		dispc_reg_out(OSD_OSDWIN1YL, DISP_YRESVGA);

		// Set OSD1 window  origin and size
		dispc_reg_out(OSD_CURXP, 0);
		dispc_reg_out(OSD_CURYP, 0);
		dispc_reg_out(OSD_CURXL, DISP_XRESVGA);
		dispc_reg_out(OSD_CURYL, DISP_YRESVGA);
		...
		// hard code to make TI demo work w/ runtime switching
		//dispc_reg_out(OSD_VIDWIN1OFST, 0x2d);
...
/* --------------^ Bruno: enable color bars V--------------- */
	...
	if (dm->osd0) {
		...
		if (register_framebuffer(info) < 0) {
			...
			printf(KERN_INFO "fb%d: %s frame buffer device\n",
